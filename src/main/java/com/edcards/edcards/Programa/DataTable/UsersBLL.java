package com.edcards.edcards.Programa.DataTable;
...
import com.edcards.edcards.Programa.DataTable.Settings.DefaultBLL;
...
import static com.edcards.edcards.Programa.Controllers.ImageController.convertByteToImage;
...
public class UsersBLL {
    ...
    public static Image getFotoUser(int id) {
        if (!existe(id)) {
            ...
            byte[] fotoBytes = (byte[]) new DefaultBLL("usuario").getOne("foto", "id", id);
            return convertByteToImage(fotoBytes);
    }
    //todo getHorario()
    public static Image getHorarioser(int id) {
        if (!existe(id)) {
            return null;
        }
        byte[] fotoBytes = (byte[]) new DefaultBLL("usuario").getOne("horario", "id", id);
        return convertByteToImage(fotoBytes);
    }
    ...
    public static Pessoa transformUser(Map<String,Object> row) {
        ...
        for (Map.Entry<String, Object> entry : row.entrySet()) {
            switch (entry.getKey()) {
                ...
                case "horario" -> pessoa.setHorario((Image) entry.getValue());
                case "foto" -> pessoa.setFoto((Image) entry.getValue());
            }
        }
